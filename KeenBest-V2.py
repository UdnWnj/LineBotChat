...
def prettyEcho(event):
    ...
    if "@客戶資訊" == event.message.text:
        ...
        sendString = save(event.message.text)
...
# DB連線資訊
def save(text):
    conn = pymssql.connect(server = '192.168.1.252',port = 1433,user = 'sa',password = 'P@ss1234',database = 'CallCenter_DEMO')
    cursor = conn.cursor()
    keyWord = text
    print(keyWord)
    sql = "SELECT  [Topic] FROM [CallCenter_DEMO].[dbo].[KnowledgeBase] where FullBaseCate = %s"
    cursor.execute(sql,keyWord)
    sqls = cursor.fetchall()
    #sqls = cursor.fetchone()
    final_result = [list(i) for i in sqls]
    print(final_result)
    thislist = []
